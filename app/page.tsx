...
export default function HomePage() {
  // دریافت محصولات و ساخت fallback در صورت undefined
  const featuredProductsRaw = getFeaturedProducts() || []
  const featuredProducts = featuredProductsRaw.length > 0 ? featuredProductsRaw.slice(0, 3) : []
          ...
          <div className="mt-12 grid grid-cols-1 gap-8 md:grid-cols-3">
            {featuredProducts.length > 0 ? (
              featuredProducts.map((product) => {
                ...
                return <ProductCard3D key={product.id} product={{ ...product, images: [firstImage] }} />
              })
            ) : (
              <p className="text-center col-span-3 text-muted-foreground">No featured products available.</p>
            )}
          </div>
        </div>
      </section>
